...
async def start():
    try:
        ...
        data = database.select_all_sentence(dbms)
        documents = []
        text = ""
        i = 0
        ...
        for sentence in tqdm(sentences):
            ...
            for value in sentence:
                text = text + value + "\n"
            doc = await upload_message(text, "sentences_" + str(i))
...
async def start_no_split():
    try:
        ...
        for value in sentences:
            text = text + value + "\n"
        document = await upload_message(text, "sentences")
...
async def start_single():
    try:
        ...
        for value in tqdm(data):
            doc = await upload_message(value, "sentences_" + str(i))
            documents.append(doc)
            i = i + 1
