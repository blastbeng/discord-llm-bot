...
pub fn get_effect_filter(effect: &str) -> Option<String> {
    match effect {
        ...
        // Demon lowers pitch to ~60% (deep, rumbling) and boosts the low end,
        // which reads clearly as "demonic" rather than just a slightly-thinner
        // normal voice (an *0.7 drop sounded too much like chipmunk).
        "demon" => Some("asetrate=44100*0.6,aresample=44100,atempo=1.6667,bass=g=12".to_string()),
...
async fn get_or_generate_tts_inner(text: &str, voice: &str, effect: &str, allow_fallback: bool) -> Result<TtsResult, Box<dyn std::error::Error + Send + Sync>> {
    ...
    if save_mp3 {
        ...
        // the background generator must never silently use a Google voice.
    ...
    // 2) GENERATE the audio (with fallback for user commands).
